...
window.poches.filters['render'] = function () {
  this.renderProductGridItem = function (displayProduct) {
    ...
    return '<a href="' + productLink + '" class="product--grid--item--four three--row w-inline-block" title="' + displayProduct['name'] + '" '+ dataAttributes.join(' ') +'><div class="product--grid--item--image random-colored-bg" style="background-color: rgba(255, 247, 220, 0.76);"><div class="product--grid--item--variant--overlay" style="background-image:' + backgroundImage + ';position:relative;">' + productImage + '</div><div class="product--quickadd"><strong class="product--quickadd--CTA">+ QUICK ADD</strong><div class="product--quickadd--sizes product--design--selector--grid flex-row"><div class="product--size--selector">XS</div><div class="product--size--selector">S</div><div class="product--size--selector">M</div><div class="product--size--selector">L</div><div class="product--size--selector">XL</div><div class="product--size--selector">XXL</div></div></div></div><h5 class="product--grid--item--title">' + productDisplayName + '</h5><div>$' + displayProduct['price'] + '</div><div class="light-gray"><u><small>See more styles</small></u></div></a>';
  };
